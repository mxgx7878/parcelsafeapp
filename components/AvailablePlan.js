...
export default function AvailablePlan() {
    ...
    const [data_Available, setData] = useState(Available_plans);
    const removeItem = (id) => {
        let arr = data_Available.filter(function (item) {
            return item.id !== id
        })
        setData(arr);
    ...
    return (
        <View style={styles.container}>
            {data_Available.map((item, index) => (
                <View style={styles.parcelsafe} key={index}>
                    <View style={styles.parcelsafeimageBox}>
                        <Image style={{ position: 'absolute', bottom: 0 }} source={item.img} />
                    </View>
